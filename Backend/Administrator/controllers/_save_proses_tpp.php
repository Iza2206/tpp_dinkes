...
try {
    if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['submit']) && $_POST['submit'] == 'Submit') {
        ...
        $insData = $mysqli->prepare("INSERT INTO dt_proses_tpp (inp_bulan, inp_tahun, id_unit, inp_nm_unit, inp_uraian,inp_rencana_bayar,inp_status) VALUES (?, ?, ?, ?, ?, ?, ?)");
        if (!$insData) {
        ...
        echo '<script language="JavaScript">document.location = "../dashboard?page=Proses_TPP_Keuangan";</script>';
