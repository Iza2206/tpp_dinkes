...
try {
    if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['submit']) && $_POST['submit'] == 'Save') {
        ...
        $updData = $mysqli->prepare("UPDATE dt_pembayaran_tpp SET inp_bulan=?, inp_tahun=?, inp_nm_unit=?, inp_uraian=?, inp_rencana_bayar=?, inp_status=? WHERE id_unit=?");
        if (!$updData) {
        ...
        echo '<script language="JavaScript">window.location.href = "../dashboard?page=Pembayaran_TPP";</script>';
